...
def secuencia_hibbard(n):
    ...
    while True:
        incremento = 2**k - 1
        if incremento > n // 3:  # MÁS RESTRICTIVO
            break
...
def secuencia_sedgewick(n):
    sec = [1]  # INCLUIR 1 EXPLÍCITAMENTE
    k = 1
    while True:
        ...
        sec.append(incremento)
        k += 1
    return sorted(sec)[::-1]  # ELIMINAR duplicados innecesarios 

def secuencia_ciura(n):
    base = [1, 4, 10, 23, 57, 132, 301, 701]  # QUITAR 1750
    sec = []   
    
    for inc in base:
        if inc <= n // 3:  # MÁS RESTRICTIVO
            sec.append(inc)
    
    # Ampliación MÁS CONSERVADORA
    if sec:
        ultimo = sec[-1]
        while ultimo * 2.25 <= n // 2:  # CONDICIÓN MÁS ESTRICTA
            nuevo_inc = round(ultimo * 2.25)
            if nuevo_inc <= n // 3 and nuevo_inc > ultimo:
                sec.append(nuevo_inc)
                ultimo = nuevo_inc
            else:
                break
    
    return sorted(sec)[::-1]  # ELIMINAR lógica redundante
